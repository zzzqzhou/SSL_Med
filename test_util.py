...
from preprocess.utils import recover_size_sitk

def test_all_case(net, image_list, num_classes, patch_size=(96, 96, 96), stride_xy=18, stride_z=4, save_result=True, test_save_path=None, preproc_fn=None, ct_foreground=None):
    total_metric = 0.0
    for image_path in tqdm(image_list, ncols=70):
        id = image_path.split('/')[-1].replace('.npz', '')
        data = np.load(image_path)
        image = data['image']
        label = data['label']
        if ct_foreground is not None:
            # assert ValueError("ct_foreground is None!")
            image = (image - ct_foreground[0]) / ct_foreground[1]
