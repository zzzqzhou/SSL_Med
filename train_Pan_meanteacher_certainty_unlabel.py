...
from dataloaders.dataset import Dataset, RandomCrop, CenterCrop, RandomRotFlip, ToTensor, TwoStreamBatchSampler

parser = argparse.ArgumentParser()
parser.add_argument('--root_path', type=str, default='./Datasets/Pancreas', help='Name of Experiment')
parser.add_argument('--exp', type=str,  default='UAMT_unlabel_Pan', help='model_name')
parser.add_argument('--max_iterations', type=int,  default=6000, help='maximum epoch number to train')
parser.add_argument('--batch_size', type=int, default=4, help='batch_size per gpu')
parser.add_argument('--labeled_bs', type=int, default=2, help='labeled_batch_size per gpu')
parser.add_argument('--base_lr', type=float,  default=0.01, help='maximum epoch number to train')
parser.add_argument('--deterministic', type=int,  default=1, help='whether use deterministic training')
parser.add_argument('--labelnum', type=int,  default=12, help='num of labeled data')
...
train_data_path = os.path.join(args.root_path, 'npz_data')
...
base_lr = args.base_lr * len(args.gpu.split(','))
...
if __name__ == "__main__":
    ...
    logging.info("Model's Params: %.3fM" % (params_num / 1e6))

    db_train = Dataset(base_dir=train_data_path,
                       split='train',
                       transform = transforms.Compose([
                        #   RandomRotFlip(), # if use RandomRotFlip on Pancreas Dataset, the DSC will drop a lot(for about 20%) :)
                          RandomCrop(patch_size),
                          ToTensor(),
                          ]))

    labelnum = args.labelnum    # default 12
    labeled_idxs = list(range(labelnum))
    unlabeled_idxs = list(range(labelnum, 60))
    ...
    for epoch_num in tqdm(range(max_epoch), ncols=70):
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            preds = torch.mean(preds, dim=0)  #(batch, 2, 96,96,96)
            uncertainty = -1.0*torch.sum(preds*torch.log(preds + 1e-6), dim=1, keepdim=True) #(batch, 1, 96,96,96)
            ...
            consistency_dist = consistency_criterion(outputs[labeled_bs:], ema_output) #(batch, 2, 96,96,96)
